...
fn main() {
    ...
    if env::var("RUSTC_IS_FINAL_STAGE").is_ok()
        && if env::var("RUSTC_MODE_STD").is_ok() {
            !rustc_snapshot
        } else {
            true
        }
    {
        ...
        // If the runtime dir environment variable is set, we add it to the search path and the
        // headers in the artifacts (e.g., libraries). If not, the compiler will search for it in
        // the default paths. During the compilation of the compiler itself, it is better to not
        // set this variable, so the path will not be hardcoded in the artifacts (although replaceable).
        if let Ok(path_symruntime) = env::var("SYMCC_RUNTIME_DIR") {
            cmd.arg("-L").arg(&path_symruntime);
            cmd.arg(&format!(
                "-Clink-arg={}",
                &format!("-Wl,-rpath,{}", path_symruntime)
            ));
        }
    }
...
fn format_rusage_data(child: Child) -> Option<String> {
    ...
    unsafe {
        ...
        try_bool!(timezoneapi::FileTimeToSystemTime(
            &user_filetime,
            &mut user_time
        ));
        try_bool!(timezoneapi::FileTimeToSystemTime(
...
fn format_rusage_data(_child: Child) -> Option<String> {
    ...
    if inblock != 0 || oublock != 0 {
        init_str.push_str(&format!(
            " fs block inputs: {} fs block outputs: {}",
            inblock, oublock
        ));
    }
